...
public class ControladorTurnosTest {
    ...
    private final long idUsuario = 1;
...
//	}
    ...
    public void CuandVoyASeleccionarTurnoMeRegresaMensajeSiNoEcuentraTurnos() {
        daadoQueNoTengoTurnosHoy();
        ModelAndView modelo = cuandoVoyASolicitarTurnos();
        SeMuestraSinTurnosDisponibles(modelo);

    }

    private void SeMuestraSinTurnosDisponibles(ModelAndView modelo) {
        verify(servicioTurnos, times(1)).buscarTurnoPorFechaDeHoy();
        assertThat(modelo.getModel().get("mensaje")).isEqualTo("Sin Turnos disponibles para la fecha. Turnos mas Proximos");
    }

    private ModelAndView cuandoVoyASolicitarTurnos() {
        return controladorTurnos.irSoliciarTurno(request);
    }

    private void daadoQueNoTengoTurnosHoy() {
        when(request.getSession()).thenReturn(session);
        when(request.getSession().getAttribute("userId")).thenReturn(1L);
        List<Turno> lista = new ArrayList<Turno>();
        when(servicioTurnos.buscarTurnoPorFechaDeHoy()).thenReturn(lista);
